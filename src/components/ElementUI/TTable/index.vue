				...
				<template v-if="!item.children">
					...
					>
						<template
							#header
							v-if="
								item.headerRequired || item.renderHeader || item.isClickEdit
							"
						>
							<render-header
...
<script>
import EditCell from '../TEditTable/EditCell.vue'
...
export default {
	...
	methods: {
		...
		rowClassNameFuc({ row, rowIndex }) {
			...
			if (
				this.isShowFirstColumn &&
				[...new Set(this.isShowFirstColumnIndex)].includes(rowIndex)
			) {
		...
		flexColumnWidth(str, arr1, index, minWidth, flag = 'max') {
			...
			if (!Array.isArray(this.table.firstColumn) && this.table.firstColumn) {
				...
				Array.isArray(this.table.firstColumn) &&
				this.table.firstColumn.length
			) {
			...
			setTimeout(() => {
				if (
					this.$refs['el-table'] &&
					this.$refs['el-table'].$el &&
					this.$refs['el-table'].$el.querySelectorAll(
						` tr > td.el-table__cell:nth-child(` + cellIndex + `) > .cell`
					).length > 0
				) {
					this.$refs['el-table'].$el
						.querySelectorAll(
